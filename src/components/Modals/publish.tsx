...
interface IModalPublishCurriculumProps {
  isPublished: boolean
  ...
  return (
    <Dialog.Root>
      <Dialog.Trigger asChild>
        <button
          className={`${isPublished ? 'bg-green-500' : 'bg-red-600'} w-full md:w-1/4 p-4 rounded-md text-center hover:bg-secondary hover:text-primary duration-300`}
        >
          {isPublished ? 'Publico' : 'Privado'}
        </button>
      </Dialog.Trigger>
      <Dialog.Portal>
        ...
        <Dialog.Content className="overflow-y-auto flex flex-col gap-4 data-[state=open]:animate-contentShow fixed top-[50%] left-[50%] max-h-[85vh] w-[90vw] max-w-[550px] translate-x-[-50%] translate-y-[-50%] rounded-[6px] bg-secondary p-4 shadow-[hsl(206_22%_7%_/_35%)_0px_10px_38px_-10px,_hsl(206_22%_7%_/_20%)_0px_10px_20px_-15px]">
          <Dialog.Title className="text-primary font-semibold text-xl">
            {isPublished ? 'Deixar Privado' : 'Deixar Público'}
          </Dialog.Title>
          ...
          <div className="w-full flex gap-4">
            ...
            <Dialog.Close asChild onClick={() => handlePublish(!isPublished)}>
